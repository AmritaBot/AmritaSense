...
> **共同基础**
> 两者都依赖 `ALIAS` 系统注册的符号名来寻址，都在编译期的 `_post_compile` 阶段完成地址解析和拼写纠错，都使用 `@markup` 装饰器管理跳转标记。理解这些共同机制，有助于掌握两者的本质差异。
...
1. **地址解析**（`_post_compile` 阶段）：在编译期将别名查表解析为 `list[int]` 绝对地址，或直接验证裸地址的有效性
...
1. **地址解析**（`_post_compile` 阶段）：在编译期将别名查表解析为 `list[int]` 绝对地址并缓存
