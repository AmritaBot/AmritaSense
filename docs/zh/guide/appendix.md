...
- **DI Cache（DI 缓存）**（v0.4.2+）：`DICache` — `WorkflowInterpreter` 内部基于 LRU 的缓存，存储已解析的依赖注入 kwargs。键为 `hash((hash(pointer), args_hash))`，避免相同节点在相同参数类型下重复解析 DI。由 unsafe 标志 `WORKFLOW_DI_NO_CACHE`、`WORKFLOW_DI_PRELOAD_CACHE` 和 `WORKFLOW_DI_PRELOAD_BATCH` 控制。
...
- **AmritaSense**：根据 **Apache 2.0** 许可证发布
- **AmritaCore**：根据 **MIT** 许可证发布（未来会更换为 Apache 2.0）
