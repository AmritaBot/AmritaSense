...
- **Pointer Cache（指针缓存）**（v0.4.3+）：`_ptr_cache` — `WorkflowInterpreter` 内部的 `LRUCache[int, list[int]]`，缓存指针推进结果。以 `hash(pointer)` 为键，存储已解析的 `base_addr`，避免 `advance_pointer()` 中的 O(D²) 回溯遍历。通过 `NO_ADDRESSING_CACHE` 禁用。
